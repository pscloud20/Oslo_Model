...
class oslo:
    ...
    def drive(self): #define a drive function for adding grain to site_0 
        ...
        self.h[0] = self.h[0] +1 #height of site_0 increases by 1
        
    def relax(self): #follows structure of mathematical algorithm of oslo model relaxation
        sites_relaxed = []
...
# =============================================================================
    ...
    def sim(self, N, animate = False, av_limit = False, av_data = False,  h1_data = False):
        for i in range(N):
            self.drive()
            self.relax()
            if animate == True:
                t.sleep(0)
            if np.all(h1_data) == True:
                self.h1_data.append(self.h[0])
            if av_limit == True: #loop that checks if the limit of avalanche reached ie grain fallen off and steady state have been reached if yes then loop breaks
    ...
    def steadystate(self):
        for i in range(len(self.h1_data)):
            if self.h1_data[i] > np.max(self.h1[len(self.h1_data())-100:]):
                return i+100
